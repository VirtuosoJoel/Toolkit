...
class LabelGui < Gtk::Window
  ...
  def initialize
    ...
    window.set_title 'Return Label Printer'
    ...
    # Checkbox to switch between Return labels and all labels on OR numbers
    @return_only = Gtk::CheckButton.new 'Return Labels only'
    default_return_only = case get_regkey_val( 'returnonly' )
    when 'true'
      true
    when 'false'
      false
    ...
    @return_only.active = default_return_only
    table.attach @return_only, 2, 3, 2, 3
    @return_only.signal_connect('toggled') { set_regkey_val( 'returnonly', @return_only.active?.to_s )  }
    
    # Label to update the user with feedback
    putsframe = Gtk::Frame.new
    @puts = Gtk::Label.new( 'Waiting for user input...' )
    table.attach putsframe, 0, 3, 3, 4
    putsframe.add @puts
  ...
  def go_go_go
    ...
    begin
      ...
      q = RDTQuery.new 'http://centrex.redetrack.com/redetrack/bin/report.php?report_locations_list=T&select_div_last_shown=&report_limit_to_top_locations=N&action=ordtrack&num=&num_raisedtrack=&status=999&pod=A&status_code=&itemtype=&location=&value_location=&tf=current&days=365&befaft=b&dd=09&mon=08&yyyy=2013&fdays=1&fbefaft=a&fdd=09&fmon=08&fyyyy=2013&ardd=09&armon=08&aryyyy=2012'
      q.set_dates( today )
      q[ 'status' ] = '' unless @return_only.active?
      ...
      keys = get_key.map do |k|
        ...
        # If it's a CNTX, leave it alone
        elsif k =~ /^CNTX/i
        
          k
          
        # If it's not an OR or CNTX number, ABORT! ABORT!
        else
